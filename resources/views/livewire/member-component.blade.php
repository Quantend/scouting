<div class="container p-6 bg-gray-100 dark:bg-gray-800 rounded-lg shadow-md">
    ...
    <form wire:submit.prevent="store" class="mt-4">
        <div class="mb-4">
            ...
            <input type="text" wire:model="name" class="border rounded p-2 w-full dark:bg-gray-700 dark:text-white dark:border-gray-600" placeholder="Voer naam in">
        ...
        @foreach ($members as $member)
            <tr class="border-t dark:border-gray-600">
                ...
                <td class="p-2 flex justify-center space-x-2">
                    <button wire:click="edit({{ $member->id }})" class="cursor-pointer bg-yellow-500 text-white p-1 rounded hover:bg-yellow-600"><flux:icon.pencil-square></flux:icon.pencil-square></button>
                    <button wire:click="confirmDelete({{ $member->id }})" class="cursor-pointer bg-red-500 text-white p-1 rounded hover:bg-red-600"><flux:icon.trash></flux:icon.trash></button>
                    <button wire:click="viewTasksFunc({{ $member->id }})" class="cursor-pointer bg-blue-500 text-white p-1 rounded hover:bg-blue-600"><flux:icon.eye></flux:icon.eye></button>
                </td>
            </tr>
        @endforeach
        </tbody>
        ...
        <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white p-6 rounded shadow-lg w-96">
                ...
                <div class="flex justify-between">
                    ...
                    <button wire:click="$set('memberToDelete', null)" class="cursor-pointer bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">
                        Annuleren
                    </button>
                </div>
            </div>
    ...
    @if ($viewTasks)
        <div class="mt-4">
            <h3 class="mb-4 text-xl font-semibold text-gray-700 dark:text-white">Taken en Totaal Uren voor {{$name}}</h3>
            <button wire:click="hideTasks()" class="cursor-pointer bg-blue-500 text-white p-2 rounded hover:bg-blue-600">Verberg Taken</button>
            <table class="w-full mt-2 border dark:bg-gray-700 dark:border-gray-600">
                ...
                @empty
                    <tr>
                        <td colspan="2" class="p-2 text-center text-gray-700 dark:text-white">Geen taken gevonden voor dit lid.</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
